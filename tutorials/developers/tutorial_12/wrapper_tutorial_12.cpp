...
int main() {
  ...
  Halide::Buffer<uint32_t> input(_COLS + 2, _ROWS/5 + 2, "input");
  
  for (int r = 0; r < _ROWS/5; r++) {
  ...
  if (rank == 4) {
    ...
    uint32_t v = _ROWS/5;
    for (int r = _ROWS/5; r < _ROWS/5 + 2; r++) {
      for (int c = 0; c < _COLS + 2; c++) {
  ...
  Halide::Buffer<uint32_t> output(_COLS, _ROWS/5, "output");
  Halide::Buffer<uint32_t> ref(_COLS, _ROWS/5, "ref");
